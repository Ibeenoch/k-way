...
const viewPost = (postId: string) => {
  ...
  return (
    <div className="sticky overflow-y-auto hide-scrollbar bg-gray-800 h-min">
       <div onClick={handleGoBack} className={`sm:mx-[25%] flex items-center py-4 pl-3 gap-3 ${ mode === 'light' ? 'bg-white text-black stroke-black' : 'bg-black text-white stroke-white'} cursor-pointer`}>
          ...
          <div className="flex ml-9 mt-4 gap-8 items-center">
            ...
            <div className={`fixed bottom-0 ${toggleControls ? 'flex' : 'hidden' } z-40 rounded-xl`}>
             <input
              ...
              isCommenting ? (
                <>
               <div className='flex items-center'><ProcessingLogo className="w-5 h-5 fill-white" /> <p className='text-sm text-white'> Commenting...</p></div> 
               </>
              ) : (
